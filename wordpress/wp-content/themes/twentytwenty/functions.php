...
//{
   ...
   $wpdb->insert('wp_contact_form',array('your_name'=>$your_name, 'your_email'=>$your_email, 'your_subject'=>$subject, 'your_message'=>$your_message 
        ),array('%s','%s','%s','%s'));
...
function display_data(){
	...
	$retrieve_data = $wpdb->get_results( "SELECT * FROM wp_contact_form" );
	foreach ($retrieve_data as $retrieved_data){ 
		$your_name =  $retrieved_data->your_name; 
		$your_email = $retrieved_data->your_email;
		$your_subject = $retrieved_data->your_subject;
		$your_message  = $retrieved_data->your_message; 

	}
		$output = '<div class="wrap">
						<h2>cotact form data</h2>
						<table>
						  <tr>
							<th>your-name</th>
							<th>your-email</th>
	...
	add_shortcode('custom_db_display','display_data');
...
function custom_field(){
	add_meta_box(
		'cars_cf',
		'Story Details',    //title of custom field
		'CF',           //custom field function
...
function CF(){
    ...
    <table>
        <tr>
            ...
            <td><input type="text" size="40" name="Price" value="<?php echo get_post_meta( get_the_ID() , 'Price', true ); ?>" /></td>
        </tr>
        <tr>
            <td style="width: 50%">Description</td>
            <td><input type="text" size="40" name="garage" value="<?php echo get_post_meta( get_the_ID() , 'Description', true ); ?>" /></td>       
        </tr>
        <tr>
...
add_action('admin_init', 'custom_field');

function save_post_data(){
    global $wpdb;
   // print_r($my_post);

    $table_name = $wpdb->prefix . 'cpt_product';    
...
/*$my_post = array(
  ...
  $wpdb->insert($table_name,$my_post,$format);
...
*/  
 ...
 // global $post;
  ...
  if (!empty($status) ){
     ...
     $wpdb->query("INSERT INTO $table_name(post_title, post_content, post_status, post_type, Price) VALUES('$title', '$content', '$status', '$post_type','$price')"); 
	
}

else {
 $wpdb->update($table_name, array('post_title' => $title,
'post_content' => $content, 'post_status' => $status, 'post_type' => 
 $post_type,'Price' => $price), array('id' =>$check_title->$id), array('%s','%s', '%s', '%s', '%s'),
 array('%d'));
...
// if ( isset( $_REQUEST['Price'] ) ) {
	// update_post_meta('cpt_product',$post->id, 'Price',sanitize_text_field($_POST["Price"]));
	// }
	////if ( isset( $_REQUEST['Price'] ) ) {
		//update_post_meta( $post_id, 'Price', sanitize_text_field( $_POST['Price'] ) );
	// }
     // $wpdb->query("UPDATE $table_name SET post_title='$title',post_content='$content',post_status='$status',post_type='$post_type',Price='$price' WHERE id=$id");
	 //$wpdb->query(" UPDATE `wp_cpt_product` SET `id`='[value-1]',`post_title`='[value-2]',`post_content`='[value-3]',`post_status`='[value-4]',`post_type`='[value-5]',`Price`='[value-6]' WHERE 1 ");
     //$wpdb->update( $table_name, array( 'post_title'=>$title,'post_content'=>$content), array( 'id' => $id ), array( '%s','%s' ) );
...
add_action( 'save_post_product', 'save_post_data', 20, 2 );
